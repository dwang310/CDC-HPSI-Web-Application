...
plot.netsim <- function(x, type = "epi", y, popfrac, sim.lines = FALSE, sims, sim.col,
  ...
  if (type == "network") {
    ...
    if (col.status == TRUE) {
      ...
      pal <- transco(c("firebrick", "steelblue", "gold", "dimgray", "green"), 0.75)
      # firebrick = #B22222BF
      # steelblue = #4682B4BF
      # gold = #FFD700BF
      # dimgray = #696969BF
      ...
      if (tea.status == TRUE) {
        ...
        cols <- ifelse(get.vertex.attribute.active(obj, "general_status", at = at) == 1,
                       pal[1], pal[2])
        #print(paste0("col1 ", cols))
        print(get.vertex.attribute.active(obj, "general_status", at = at))
        cols <- ifelse(get.vertex.attribute.active(obj, "general_status", at = at) == 2,
                       pal[3], pal[2])
        #print(paste0("col2 ", cols))
        #print(get.vertex.attribute.active(obj, "general_status", at = at))
        cols <- ifelse(get.vertex.attribute.active(obj, "general_status", at = at) == 3,
                       pal[4], pal[2])
        #print(paste0("col3 ", cols))
      }
